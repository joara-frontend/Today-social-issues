...
13초씩 대기한다. 기사 한 건의 요약이 `ApiError`(500/502/503)로 실패하면
일시적 오류로 보고 3초 대기 후 최대 2회까지 재시도하고, 그 외 실패는
`try/catch`로 개별 처리해 전체 파이프라인은 계속 진행된다.

`429`는 재시도하지 않는다. 무료 티어의 429는 분당 제한이 아니라
`GenerateRequestsPerDayPerProjectPerModel-FreeTier`(하루 20회) 쿼터 초과라서,
한 번 발생하면 남은 기사도 전부 실패할 게 확실하기 때문이다. 재시도로 쿼터를
더 깎는 대신 즉시 전체 수집을 조기 종료한다.

> **알려진 제한사항**: 재시도는 500/502/503 같은 일시적 오류에만 적용된다.
> Gemini 응답의 JSON 파싱 실패 등 다른 원인이거나 재시도를 모두 소진한
> 경우에는 해당 기사가 스킵되며, RSS 풀에서 대체 기사를 다시 채우지 않으므로
> 카테고리당 3개 미만으로 저장될 수 있다. 하루 쿼터(20회)를 넘기면 그 날은
> 이후 카테고리/기사가 전부 스킵된다.
