...
import { CATEGORY_MAP, SITE_NAME } from "@/shared/config";
...
export async function generateMetadata({ params }: IssueDetailPageProps) {
  ...
  if (!issue) return {};
  return { title: `${issue.title} | ${SITE_NAME}` };
}
...
export default async function IssueDetailPage({
  ...
  return (
    <article className="mx-auto max-w-2xl px-4 pt-8 pb-24 md:px-6 md:pt-11">
      <Link
        href="/"
        className="text-text-muted duration-base hover:text-text-primary mr-2 mb-6 inline-flex items-center gap-1 text-sm font-semibold transition-colors"
      >
        ← 목록으로
      </Link>
